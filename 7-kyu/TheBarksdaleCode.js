...
const decode = (codedNum) => {
	const decodedNums = {1: 9, 2: 8, 3: 7, 4: 6, 5: 0, 6: 4, 7: 3, 8: 2, 9: 1, 0: 5};
  const keys = Object.keys(decodedNums);
  let decoded = '';
  codedNum.split('').forEach((item) => {
    keys.forEach((key) => {
    	if (item === key) {
      	decoded += `${decodedNums[key]}`;
      }
    });
  });
  return decoded;
